...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      area_code,
      months,
    } = body as { area_code?: string; months?: number };

    if (!area_code || typeof area_code !== "string") {
      return NextResponse.json({ success: false, error: "area_code is required" }, { status: 400 });
    }
    ...
    const api = await smsbusRent.getNumber(area.area_code, m);

    if (api.code !== 200 || !api.data) {
      await User.findByIdAndUpdate(session.userId, { $inc: { walletBalance: amountInr } });
      return NextResponse.json(
        { success: false, error: api.message || "Rental failed", data: api },
        { status: 400 }
      );
    }
    ...
    await Transaction.create({
      ...
      type: "debit",
      amount: amountInr,
      description: `Long-term rental — ${area.area_title} (${m} mo)`,
      status: "completed",
    });
